...
public class RSA {
    ...
    public static BigInteger convertFromASCII(String text) {
    	...
    	BigInteger block = new BigInteger(text.getBytes());
    	return block;
    	/*
    	BigInteger ret = BigInteger.valueOf(0);
    	char[] message = text.toCharArray();
    	ArrayList<Integer> ascii_value = new ArrayList<Integer>();
    	
    	for(char c : message) {
    		ascii_value.add((int) c);
    	}
    	
    	String concat = "";
    	for(Integer i : ascii_value) {
    		concat += i.toString();
    	}
    	
    	ret = BigInteger.valueOf(Integer.parseInt(concat));
    ...
    public static String convertToASCII(BigInteger block) {
        // your code here; a dummy return statement is put below to allow this to compile
    	String ascii = new String(block.toByteArray());
    	return ascii;
    	/*
    	String num = block.toString();
    	String ret = "";
    	String temp = "";
    	int ascii = 0;
    	...
    	for(int i=0; i < num.length(); i++) {
    		if(num.charAt(i) == '1') {
    			temp = temp + num.charAt(i) + num.charAt(i+1) + num.charAt(i+2);
    			...
    			temp = temp + num.charAt(i) + num.charAt(i+1);
    			ascii = Integer.parseInt(temp);
    			ret = ret + (char)ascii;
    			temp = "";
    			ascii = 0;
    ...
    public static RSAKey generateKeys (int bitLength, int seed) throws Exception {
        ...
        BigInteger p = new BigInteger(bitLength, 80, random);
        BigInteger q = new BigInteger(bitLength, 80, random);
        
        while(p == q) 
        	q = new BigInteger(bitLength, 80, random);
        
        BigInteger e_prime = new BigInteger((2*bitLength) - 2, 80, random);
        
        RSAKey key = new RSAKey();
        key.bitLength = bitLength;
        key.n = p.multiply(q);
        key.e = e_prime;
        
        BigInteger one = BigInteger.valueOf(1);
        BigInteger pmone = p.subtract(one);
        BigInteger qmone = q.subtract(one);
        key.d = key.e.modInverse(pmone.multiply(qmone));
        return key;
    }
    ...
    public static CipherText encrypt (RSAKey key, String plainText) throws Exception {
        // your code here; a dummy return statement is put below to allow this to compile
    	CipherText enc = new CipherText();
    	enc.fileLength = plainText.length();
    	int x = key.n.bitLength();
    	enc.blockLength = (x-1)/8;
    	
    	String temp = "";
    	
    	while(plainText.length() != 0) {
    		if(plainText.length() < enc.blockLength) {
    			for(int i=0; i < plainText.length(); i++) {
	    			temp = temp + plainText.charAt(i);
	    		}
    		}
    		else {
	    		for(int i=0; i < enc.blockLength; i++) {
	    			temp = temp + plainText.charAt(i);
	    		}
    		}
    		
    		BigInteger c = convertFromASCII(temp);
    		c = c.modPow(key.e, key.n);
    		enc.encryptedBlocks.add(c);
    		temp = "";
    		...
    		if(plainText.length() < enc.blockLength) {
    			...
    			plainText = plainText.substring(enc.blockLength, plainText.length());
    		}
    	}
    ...
    public static String decrypt (RSAKey key, CipherText cipherText) throws Exception {
    	...
    	for(BigInteger i: cipherText.encryptedBlocks) {
    		i = i.modPow(key.d, key.n);
    		temp = convertToASCII(i);
    		dec += temp;
    		temp = "";
    ...
    public static RSAKey crack (RSAKey key) throws Exception {
        // your code here; a dummy return statement is put below to allow this to compile
    	BigInteger start = key.n.sqrt();
    	if(start.mod(BigInteger.valueOf(2)).equals(BigInteger.valueOf(0))) {
    		start = start.add(BigInteger.valueOf(1));
    	}
    	BigInteger one = BigInteger.valueOf(1);
    
    	for(int i=0; ;i++) {
    		if(key.n.mod(start).equals(BigInteger.valueOf(0))) {
    			break;
    		}
    		start = start.subtract(BigInteger.valueOf(2));
    		System.out.println(start);
    	}
    	
    	BigInteger q = key.n.divide(start);
    	BigInteger d = key.e.modInverse((start.subtract(one)).multiply((q.subtract(one))));
    	key.d = d;
        return key;
    }
    ...
    public static void main (String[] args) throws Exception {
        ...
        int seed = 0;
        //TESTTTTT
        BigInteger f = BigInteger.valueOf(100101);
        System.out.println(convertToASCII(f));
        System.out.println(convertFromASCII(convertToASCII(f)));
        System.out.println(convertFromASCII("de"));
        
        String text = "Sun is down, freezin' cold\n" + 
        ...
        RSAKey k = generateKeys(15, 0);
        CipherText c = encrypt(k, text);
        //System.out.println(decrypt(k,c));
        
        RSAKey cracked = crack(k);
        System.out.println(decrypt(cracked,c));
